      ...
      <div className="container mx-auto px-6 relative z-10">
        <div ref={headerRef} className="max-w-4xl mx-auto text-center mb-16">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={headerInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="text-subtle/50 font-inter text-base md:text-lg mt-6 max-w-2xl mx-auto">
            Explore the TAKKERU Cart menu — Boba Tea, Signature Ramen, Mandu and Tteokbokki made for bold street-food experiences.
          </motion.p>
        </div>

        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {MENU_PRODUCTS.map((product, index) => (
            <FoodMenuCard key={product.id} product={product} index={index} number={NUMBERS[index]} inView={gridInView} onAdd={(id) => addItem(id)} onNavigate={(id) => navigate(`/product/${id}`)} />
          ))}
        </div>
      </div>
    </section>
...
function FoodMenuCard({ product, index, number, inView, onAdd, onNavigate }) {
  return (
    <motion.div initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: index * 0.12, ease: 'easeOut' }} className="group relative rounded-2xl border border-white/5 overflow-hidden bg-white/[0.02] transition-colors duration-300 hover:border-accent">
